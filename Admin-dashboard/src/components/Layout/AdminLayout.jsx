...
const menuItems = [
  ...
  { key: '/students',    icon: <UsergroupAddOutlined />, label: 'Học viên' },
  { key: '/enrollments', icon: <TeamOutlined />,         label: 'Đăng ký' },
  { key: '/categories',  icon: <TagsOutlined />,         label: 'Danh mục' },
  { key: '/quizzes',     icon: <CheckSquareOutlined />,  label: 'Trắc nghiệm' },
...
const AdminLayout = () => {
  ...
  const sidebarContent = (isDrawer = false) => (
    <div style={{
      ...
      {/* Logo */}
      <div
        onClick={() => { navigate('/'); if (isDrawer) setDrawerOpen(false); }}
        style={{
          height: 64,
          display: 'flex',
          alignItems: 'center',
      ...
      {/* Bottom profile */}
      <div
        onClick={() => { navigate('/profile'); if (isDrawer) setDrawerOpen(false); }}
        style={{
