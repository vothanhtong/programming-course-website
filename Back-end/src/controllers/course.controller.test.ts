...
const mockAllModels = () => {
  ...
  vi.doMock('../models/course.models/review.model',     () => ({ default: { find: vi.fn(), create: vi.fn(), findByIdAndUpdate: vi.fn(), findByIdAndDelete: vi.fn(), countDocuments: vi.fn(), deleteMany: vi.fn() } }));
  vi.doMock('../models/course.models/enrollment.model', () => ({ default: { find: vi.fn(), create: vi.fn(), findByIdAndUpdate: vi.fn(), countDocuments: vi.fn(), aggregate: vi.fn() } }));
  vi.doMock('../models/student.model',                  () => ({ default: { updateOne: vi.fn() } }));
};
...
describe('postEnroll - validation', () => {
  ...
  it('201 và cập nhật enrolledCourses khi có studentId', async () => {
    ...
    vi.doMock('../models/course.models/enrollment.model', () => ({ default: { create: vi.fn().mockResolvedValue({ _id: 'enroll-id' }) } }));
    vi.doMock('../models/student.model',                  () => ({ default: { updateOne: studentUpdateMock } }));
    vi.doMock('../models/course.models/category.model',   () => ({ default: {} }));
    vi.doMock('../models/course.models/lesson.model',     () => ({ default: {} }));
    vi.doMock('../models/course.models/review.model',     () => ({ default: {} }));
    const { postEnroll } = await import('./course.controller');
    const req = mockReq({ body: { courseId: '507f1f77bcf86cd799439011', studentName: 'Test', studentEmail: 'test@test.com' } });
    (req as any).studentId = 'student-abc';
    const res = mockRes();
    await postEnroll(req, res);
    expect(res.status).toHaveBeenCalledWith(201);
    expect(studentUpdateMock).toHaveBeenCalledWith({ _id: 'student-abc' }, { $addToSet: { enrolledCourses: '507f1f77bcf86cd799439011' } });
  });
});
...
describe('adminAddCourse - validation', () => {
  ...
  it('400 khi thiếu title', async () => {
    ...
    expect(res.status).toHaveBeenCalledWith(400);
    expect(res.json).toHaveBeenCalledWith({ message: 'Thiếu title, category hoặc price' });
  });
